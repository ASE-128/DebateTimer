...
**版本：2.11.3** | 作者：Chen Yu | 许可证：GPL-3.0
...
- **导出独立计时器**：将当前配置与 Electron 运行时打包为单个安装程序 EXE，双击后按向导安装到本地，自动创建桌面和开始菜单快捷方式，安装完成后直接启动；赛前可修改队伍名称与辩题，不可修改其他配置。无需目标电脑安装 Node.js
...
项目内置 `electron-builder` 配置，支持生成 Windows 安装包（NSIS）与便携版（Portable）。
...
构建输出位于 `dist/` 目录（安装包与便携版的命名会包含版本号）。

### 独立计时器安装包（应用内导出）

在编辑页点击"导出独立计时器"，应用会将当前配置和 Electron 运行时打包为一个 NSIS 安装程序 EXE。运行该安装程序会：
