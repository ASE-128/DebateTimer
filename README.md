...
**版本：2.1.0** | 作者：Chen Yu | 许可证：GPL-3.0
...
- **主题样式**：自定义背景、正反方颜色、标题色、字体与字号缩放；支持上传自定义字体文件。
- **导入/导出配置**：将当前比赛配置保存为 JSON 文件，或从外部 JSON 文件导入配置。
- **导出独立计时器**：将当前配置与 Electron 运行时打包为单个 EXE 可执行文件（WinRAR 自解压），双击即可启动；赛前可修改队伍名称与辩题，不可修改其他配置。
...
- **主题与样式**：选择背景类型/颜色、调整正反方颜色、标题色、字体与字号；上传自定义字体文件。卡片内容过多时可独立滚动。
- **环节列表**：
...
npm run dist
# 输出目录：dist/
# DebateTimer Setup 2.1.0.exe —— 安装包
# DebateTimer 2.1.0.exe —— 便携版
```

### 独立 EXE 导出（应用内）

在编辑页点击“导出独立计时器”，应用会将当前配置和 Electron 运行时打包为一个自解压 EXE，无需目标电脑安装 Node.js 或 WinRAR。
