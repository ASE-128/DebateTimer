...
async function switchPreset(preset, colorMode) {
  const resolved = window.DataService?.resolvePreset ? await window.DataService.resolvePreset(preset, colorMode) : null;
  if (!resolved) return;
...
async function loadConfig() {
  const config = await window.DataService.loadConfig();
  fillEditorUI(config);
}

async function showMigrationNoticeIfNeeded() {
  if (!window.DataService?.consumeMigrationInfo) return;
  try {
    const info = await window.DataService.consumeMigrationInfo();
    if (!info) return;
...
async function saveConfig() {
  ...
  log('info', '保存配置');
  await window.DataService.saveConfig(gatherConfig());
...
async function resetConfig() {
  log('info', '重置配置');
  await window.DataService.resetConfig();
  await loadConfig();
...
dom.importConfigBtn.addEventListener('click', async () => {
  log('info', '导入配置');
  const result = await window.DataService.importConfig();
  if (result?.ok) {
...
dom.exportConfigBtn.addEventListener('click', async () => {
  log('info', '导出配置');
  const result = await window.DataService.exportConfig(gatherConfig());
  if (result?.ok) {
...
if (window.DataService?.onExportProgress) {
  window.DataService.onExportProgress(({ percent, message }) => {
...
dom.exportTimerBtn.addEventListener('click', async () => {
  ...
  try {
    const result = await window.DataService.exportStandalone(gatherConfig());
    if (result?.ok) {
      log('info', `独立计时器已导出: ${result.path}`);
      let changelogLine = '';
      try {
        const summary = await window.DataService.getLatestChangelog();
        if (summary) {
...
dom.openTimerBtn.addEventListener('click', () => window.DataService.openTimer());
...
async function showAbout() {
  ...
  try {
    const version = await window.DataService.getAppVersion();
...
async function loadAndRenderTemplates() {
  ...
  try {
    const templates = await window.DataService.getTemplates();
...
async function applyTemplateById(id) {
  try {
    const result = await window.DataService.applyTemplate(id);
...
async function deleteTemplateById(id) {
  ...
  if (!confirmed) return;
  try {
    const result = await window.DataService.deleteTemplate(id);
    if (!result?.ok) {
...
async function saveCurrentAsTemplate() {
  if (currentEditingTemplateId) {
    try {
      ...
      const result = await window.DataService.saveTemplate(currentEditingTemplateId, name, '', gatherConfig());
      if (!result?.ok) {
  ...
  try {
    const result = await window.DataService.saveTemplate(id, name.trim(), '', gatherConfig());
    if (!result?.ok) {
      alert(`保存失败：${result?.error || '未知错误'}`);
      return;
    }
...
function showUpdateNotification(version) {
  ...
  if (window.DataService?.getLatestChangelog) {
    window.DataService
      .getLatestChangelog()
      .then((summary) => {
        if (updateChangelog) {
...
if (window.DataService?.onUpdateAvailable) {
  window.DataService.onUpdateAvailable(({ version }) => {
    log('info', `收到可用更新: ${version}`);
    showUpdateNotification(version);
  });
}

if (window.DataService?.onUpdateDownloaded) {
  window.DataService.onUpdateDownloaded(() => {
    log('info', '更新已下载');
...
if (window.DataService?.onUpdateError) {
  window.DataService.onUpdateError(({ message }) => {
...
updateDownloadBtn?.addEventListener('click', async () => {
  if (!window.DataService?.startDownloadUpdate) return;
  updateDownloadBtn.textContent = '下载中...';
  updateDownloadBtn.disabled = true;
  try {
    await window.DataService.startDownloadUpdate();
  } catch (e) {
...
updateRestartBtn?.addEventListener('click', () => {
  if (!window.DataService?.quitAndInstall) return;
  window.DataService.quitAndInstall();
});
...
updateSkipBtn?.addEventListener('click', () => {
  if (window.DataService?.skipUpdate && pendingUpdateVersion) {
    window.DataService.skipUpdate(pendingUpdateVersion);
  }
  hideUpdateNotification();
});
